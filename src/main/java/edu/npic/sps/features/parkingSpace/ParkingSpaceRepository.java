...
public interface ParkingSpaceRepository extends JpaRepository<ParkingSpace, Integer> {
    ...
    @Query("select p from ParkingSpace p where p.site.uuid in ?1")
    List<ParkingSpace> findBySite_UuidIn(Collection<String> uuids);

    @Query("select count(p) from ParkingSpace p where p.site.uuid in ?1")
    Integer countBySite_UuidIn(Collection<String> uuids);
